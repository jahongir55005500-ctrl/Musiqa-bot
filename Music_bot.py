...
def start(message):
    if check_sub(message.from_user.id):
        ...
        bot.send_message(message.chat.id, "⚠️ Obuna bo'ling:", reply_markup=markup)
...
def search(message):
    ...
    except: bot.edit_message_text("❌ Topilmadi yoki YouTube blokladi.", message.chat.id, m.message_id)
...
def h(c):
    ...
    else: bot.answer_callback_query(c.id, "❌ Obuna bo'lmadingiz!", show_alert=True)
